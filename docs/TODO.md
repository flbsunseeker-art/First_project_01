...
当前进入 v0.2「可信日常使用版」，目标是每天看清资产状态，并在交易后完成记录与复盘。

### P0：数据可信

- [x] 修复短周期行情和汇率缺口未触发刷新问题。
- [x] 实时汇率失败时使用最近缓存汇率，不静默使用固定默认值。
- [x] 依据 2026-09-18 最新持仓完成交易校准并回填历史快照。
- [ ] 明确“最近应有交易日”口径，补充 A 股、港股、美股节假日处理。
- [ ] 前端 API 失败时展示异常状态，移除伪装为正常结果的零值/空列表兜底。
- [ ] 数据状态展示行情、汇率、组合快照三个独立截止时间。

### P0：账本输入闭环

- [ ] 增加期初持仓新增与修改界面。
- [ ] 在前端补齐交易编辑能力，并从原交易日期开始重算。
- [ ] 交易流水支持按证券、日期、理由筛选，并展示备注。
- [ ] 为移动端增加交易历史卡片；当前通用表格在窄屏会被隐藏。
- [ ] 明确券商摊薄成本与 StockPilot 移动平均成本的差异说明。

### P0：复盘闭环

- [ ] 收益日历支持点击日期进入当日详情。
- [ ] 单日详情展示当日盈亏、买卖现金流和个股贡献。
- [ ] 从单日详情关联当天交易理由与备注。

### P1：界面效率

- [ ] 首页首屏弱化重复品牌介绍，把数据新鲜度和核心资产指标前置。
- [ ] 修复移动端持仓数量与排序控件挤压成竖排的问题。
- [ ] 保持桌面高密度信息展示，同时为移动端使用专用卡片结构。
- [ ] 补齐加载态、异常态、空态及键盘焦点可见性检查。
